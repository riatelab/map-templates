...
box_move_and_resize <- function(boxes, x, x_target, x_target_add = TRUE){
  ...
  if (class(st_geometry(x))[1]=="sfc_POINT"){
    out <- ptbox_move_and_resize(boxes = boxes, x = x, x_target = x_target, x_target_add = TRUE)
    return(out)
  }
...
ptbox_move_and_resize <- function(boxes, x, x_target, x_target_add){
  ...
  # intersect mask and x
    sel <- suppressWarnings(st_collection_extract(
